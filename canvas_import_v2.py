...
def extract_canvas_pages(docx_file):
    ...
    for para in doc.paragraphs:
        ...
        if "---" in text or "horizontal" in style:
            if current["title"]:
...
def create_quiz(domain, course_id, title, html, token, questions):
    ...
    r = requests.post(url, headers=headers, json=quiz_payload)
...
def parse_quiz_questions(raw_text):
    questions = []
    blocks = re.findall(r"<question><(.*?)>\s*(.*?)</question>", raw_text, re.DOTALL)
    ...
    for i, block in enumerate(pages):
        ...
        if st.button(f"Upload '{title}'", key=i):
            ...
            if page_type == "page":
                ...
                questions = parse_quiz_questions(block["content"])
                ref = create_quiz(canvas_domain, course_id, title, html, token, questions)
                success = add_to_module(canvas_domain, course_id, mid, "Quiz", ref, title, token)
            else:
